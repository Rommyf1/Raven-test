...
const verifyInputFormat = (expresionToValidate, event) => {
  ...
  expectedValues.test(keyPressed) ? keyPressed : event.preventDefault();
};
...
const hasInputWrongLength = (valueProvided, minLength, maxLength) => {
  if (valueProvided.length < minLength || valueProvided.length > maxLength) {
    ...
    personName.classList.add(`input-error`);
    personName.focus();
    return true;
  }
  return false;
...
const hasInputWrongNumber = (valueProvided, minLength, maxLength) => {
  if (
    parseInt(valueProvided) >= minLength &&
    parseInt(valueProvided) <= maxLength
  ) {
    return false;
  }
  alert(`Error en el Elemento Edad`);
  personAge.classList.add(`input-error`);
  personAge.focus();
  return true;
};
...
const verifyFormData = (event) => {
  ...
  if (
    hasInputWrongLength(name.value, minLength, maxLength) ||
    hasInputWrongNumber(age.value, minAge, maxAge)
  ) {
...
personName.addEventListener(`keypress`, (event) =>
  verifyInputFormat(inputFieldFormats.name, event)
);
personAge.addEventListener(`keypress`, (event) =>
  verifyInputFormat(inputFieldFormats.age, event)
);
